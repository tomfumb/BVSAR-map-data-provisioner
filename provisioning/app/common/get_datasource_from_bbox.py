...
def get_datasource_from_bbox(bbox: BBOX, output_dir: str) -> None:
    ...
    srs.SetFromUserInput(BBOX_CRS_CODE)
    if not layer:
        layer = datasource.CreateLayer(BBOX_LAYER_NAME, srs, ogr.wkbPolygon)
    if layer.GetFeatureCount() == 0:
        geometry = ogr.CreateGeometryFromWkt(f"POLYGON (({bbox.min_x} {bbox.min_y},{bbox.max_x} {bbox.min_y},{bbox.max_x} {bbox.max_y},{bbox.min_x} {bbox.max_y},{bbox.min_x} {bbox.min_y}))")
