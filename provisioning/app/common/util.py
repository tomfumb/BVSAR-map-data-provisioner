...
def get_run_data_path(run_id: str, path_parts: Tuple[str] = None) -> str:
    return os.path.join(
        *(get_base_path(), "run", run_id, *(path_parts if path_parts else list()))
    )
...
def get_export_path(path_parts: Tuple[str] = None) -> str:
    return os.path.join(
        *(get_base_path(), "export", *(path_parts if path_parts else list()))
    )


def get_result_path(path_parts: Tuple[str] = None) -> str:
    return os.path.join(
        *(get_base_path(), "result", *(path_parts if path_parts else list()))
    )
...
def swallow_unimportant_warp_error(ex: Exception) -> None:
    if re.match(
        r"Attempt to create 0x\d+ dataset is illegal\,sizes must be larger than zero",
        str(ex),
    ):
        logging.debug("Bouding box intersects too few pixels to clip a new image")
    else:
        raise ex
