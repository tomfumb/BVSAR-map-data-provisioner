...
def retrieve_directory(domain: str, path: str) -> str:
    cache_directory = get_cache_path((re.sub(r"[^a-z0-9\.]", "", f"{domain}{path}", flags=re.IGNORECASE),))
        ...
        if not ls_type.startswith("d"):
            ...
            if os.path.exists(destination_path):
                ...
                if local_size != remote_size:
                    logging.info(f"Local {name} is incorrect size, retrieving again {local_size}/{remote_size}")
