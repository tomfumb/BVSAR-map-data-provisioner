...
def add_or_update(source_dir: str, dest_dir: str):
    ...
    for zoom_dir in [
        ...
        if os.path.isdir(os.path.join(source_dir, entry_name))
    ]:
        x_dirs = list(
            map(
                ...
                sorted(
                    [
                        int(z_entry)
                        for z_entry in os.listdir(os.path.join(source_dir, zoom_dir))
                    ]
                ),
            )
        )
        edge_tiles += list(
            map(
                ...
                os.listdir(os.path.join(source_dir, zoom_dir, x_dirs[0])),
            )
        )
        if len(x_dirs) > 1:
            edge_tiles += list(
                map(
                    ...
                    os.listdir(os.path.join(source_dir, zoom_dir, x_dirs[-1])),
                )
            )
            if len(x_dirs) > 2:
                    ...
                    y_file_nums = sorted(
                        [
                            ...
                            for y_file_name in os.listdir(
                                os.path.join(source_dir, zoom_dir, x_mid_dir)
                            )
                        ]
                    )
                    edge_tiles.append(
