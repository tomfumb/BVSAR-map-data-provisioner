...
def execute(bbox: BBOX, run_id: str, xyz_url: str, profile_name: str, layers: List[ProjectLayer]) -> None:
    xyz_result = xyz_provisioner(bbox, xyz_url, ZOOM_MIN, ZOOM_MAX, "image/jpeg", OUTPUT_FORMAT) 
    generate_result = generate_tiles(layers, ["common", profile_name], bbox, profile_name, ZOOM_MIN, ZOOM_MAX, run_id)
    xyz_tiles_merged = list()
    logging.info("Merging generated tiles to xyz base")
    for data_tile in generate_result.tile_paths:
        matching_raw_xyz_tile = data_tile.replace(generate_result.tile_dir, xyz_result.tile_dir)
        existing_merge_tile = data_tile.replace(generate_result.tile_dir, get_result_path((profile_name,)))
        merge_xyz_tiles(existing_merge_tile if os.path.exists(existing_merge_tile) else matching_raw_xyz_tile, data_tile, data_tile)
    ...
    for xyz_tile in xyz_result.tile_paths:
        if xyz_tile not in xyz_tiles_merged:
            xyz_copy_path = xyz_tile.replace(xyz_result.tile_dir, generate_result.tile_dir)
