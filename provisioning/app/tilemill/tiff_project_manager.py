...
from app.tilemill.ProjectDefinition import ProjectDefinition

def getProjectDefinition(name: str, bbox: BBOX, mss_path: str) -> ProjectDefinition:
    session = requests.session()
    # existingProjects = list(filter(lambda project: project.get("id") == name, session.get("{url}/api/Project".format(url = environmentConfig.get("tilemillUrl"))).json()))
    # if len(existingProjects) > 0:
    #     logging.info("Project {projectName} already in use, will be replaced".format(projectName = projectName))
    centre = ogr.CreateGeometryFromWkt(F"POLYGON (({bbox.min_x} {bbox.min_y}, {bbox.max_x} {bbox.min_y}, {bbox.max_x} {bbox.max_y}, {bbox.min_x} {bbox.max_y}, {bbox.min_x} {bbox.min_y}))").Centroid().GetPoint()
    zoomLevels = list()
