...
def create_or_update_project(
    tilemill_url: str, project_properties: ProjectCreationProperties
) -> None:
    ...
    project = {
        ...
        "Stylesheet": [
            {"id": str(i), "data": content}
            for i, content in enumerate(project_properties.mss)
        ],
        "Layer": list(
            map(
                lambda project_layer: _convert_project_layer_to_layer(project_layer),
                project_properties.layers,
            )
        ),
        "scale": 1,
        "metatile": 2,
...
def request_export(tilemill_url: str, project_properties: ProjectProperties) -> str:
    ...
    export_filename = "{project_name}_{unique_part}.mbtiles".format(
        project_name=project_properties.name,
        unique_part=re.sub("[^a-z0-9]", "", str(uuid.uuid4()), flags=re.IGNORECASE),
    )
    exportDefinition = {
        "progress": 0,
        "status": "waiting",
        ...
        try:
            ...
            for status in statuses:
                ...
                if status_fileame == export_filename:
                    ...
                    logging.info(
                        f"Project {project_properties.name} progress {progress * 100}%, remaining: {remaining}ms"
                    )
                    if (
                        progress > 0 and remaining == 0
                    ):  # check both as a race condition in tilemill appears to permit 0 remaining when nothing has started yet
                        return export_filename
                    else:
                        time.sleep(
                            min(10, sys.maxsize if remaining == 0 else remaining / 1000)
                        )
...
def _convert_project_layer_to_layer(project_layer: ProjectLayer) -> Dict[str, object]:
    ...
    if project_layer.type.value not in bbox_calculators:
        raise Exception(
            f"Do not understand {project_layer.type.value}, unable to calculate BBOX"
        )
    layer_bbox = bbox_calculators[project_layer.type.value](
        project_layer.path, project_layer.crs_code
    )
    return {
        "geometry": project_layer.type.value,
        "extent": {
            "minX": layer_bbox.min_x,
            "minY": layer_bbox.min_y,
            "maxX": layer_bbox.max_x,
            "maxY": layer_bbox.max_y,
        },
        "id": layer_id,
...
def _getExtentFromRaster(path: str, crs_code: str) -> BBOX:
    ...
    return BBOX(
        min_x=upperLeft[0], min_y=lowerRight[1], max_x=lowerRight[0], max_y=upperLeft[1]
    )
