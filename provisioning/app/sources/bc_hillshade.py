...
def provision(bbox: BBOX):
    ...
    for cell_part_name, cell_data in bbox_cells.items():
        ...
        if not os.path.exists(dem_name) or os.stat(dem_name).st_size == 0:
        ...
        if not os.path.exists(hillshade_name) or os.stat(hillshade_name).st_size == 0:
